...
(function () {
  ...
  function updateCartBadge() {
    ...
    document.querySelectorAll('a[href="/MicoVell/cart"]').forEach(link => {
      ...
      if (cart.length === 0) {
        main.innerHTML = `
          <div style="max-width:600px;margin:4rem auto;text-align:center;padding:2rem">
            ...
            <a href="/MicoVell/shop" style="display:inline-block;background:#ea580c;color:#fff;padding:0.75rem 2rem;border-radius:8px;text-decoration:none;font-weight:600;transition:background 0.2s">Start Shopping →</a>
          </div>`;
        return;
      }
      ...
      function renderItems() {
        ...
        itemsEl.innerHTML = c.map(item => `
          <div data-id="${item.id}" style="display:flex;align-items:center;gap:1rem;padding:1rem;border:1px solid var(--border,#e5e7eb);border-radius:12px;margin-bottom:1rem;background:var(--card,#fff)">
            <img src="${item.image}" alt="${item.name}" style="width:80px;height:80px;object-fit:cover;border-radius:8px;flex-shrink:0" onerror="this.src='/MicoVell/images/hero-mushroom.webp'">
      ...
      function renderSummary() {
        ...
        summaryEl.innerHTML = `
          <div style="border:1px solid var(--border,#e5e7eb);border-radius:12px;background:var(--card,#fff);padding:1.5rem;position:sticky;top:80px">
            ...
            <a href="/MicoVell/checkout" style="display:block;width:100%;text-align:center;background:#ea580c;color:#fff;padding:0.875rem;border-radius:10px;text-decoration:none;font-weight:700;font-size:1rem;transition:background 0.2s" onmouseover="this.style.background='#c2410c'" onmouseout="this.style.background='#ea580c'">Proceed to Checkout →</a>
          </div>`;
      }
